<template>
 ...
 <script>
    import Vue from 'vue'
    import TestCustomActions from './CsStatusCustomActions.vue'
    export default 
    {   props: {  ticketstatus: { type: Object, default() { return null; }  },   },
        data () {  return { seen: true  } },
        created() { console.log('cs/csstatus/csstatusListview.vue---- Component created.')   },
        updated() { console.log('cs/csstatus/csstatusListview.vue--- Component updated.')   },
        components: {'test-list-view-actions': TestCustomActions, },
        mounted() { console.log('cs/csstatus/csstatusListview.vue--TestListView.vue- Component mounted.')  },
        methods: 
          { onClickNew() //--------add----------1
              { console.log('cs/csstatus/csstatusListview.vue----onClickNew');
                let formData = {   id: '',  STATUS: '',  comment: '',  };
                let payload = { isShow: true, data: {action: 'Add', data: formData, index: 0} };
                this.$store.dispatch('csstatusshowpopup', payload)  //----triggers this in store--with empty data and opens new popup for adding
              },
            onActions(data) //------------edit 2---coming from custom actions
              { console.log('cs/csstatus/csstatusListview.vue--onActions data=', data);
                ...
                {   let swal = this.$swal; let me = this;
                    this.$swal({
                             ...
                             {  me.$store.dispatch('deletestatus', data.data)
                                .then((response) => {})
                                .catch((error) => {});
                             }, function (dismiss) {  });
                    return;
                }
                this.$store.dispatch('csstatusshowpopup', payload) //----edit 3--its here becasue if above delete--popup comes
